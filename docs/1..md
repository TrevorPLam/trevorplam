...
Work through `TASKS.md` and resolve the first pending task, one task at a time, with production‑quality changes. Do NOT use TODO.md. 
...
1. `TASKS.md` – the master task list with detailed acceptance criteria, dependencies, and anti‑patterns.
...
2. `TASKS.md` (the task specification)
...
1.  **Read `TASKS.md` in Full:** Understand the complete list of tasks and identify the first task marked `pending`.
...
- If you notice related low‑risk problems in the same area, either:
  ...
  - add them to `TASKS.md` as follow‑up items for later.
...
- Update `TASKS.md` if the current description is wrong or incomplete for the area you changed.
...
Only mark a task complete in `TASKS.md` after:
